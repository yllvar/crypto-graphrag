...
class TestGraphitiClient:
    ...
    def test_initialize_schema(self, mock_neo4j_driver):
        ...
        # Verify that session.run was called with the expected constraints and indexes
        # We expect at least one call for constraints and one for indexes
        assert mock_sync_session.run.call_count >= 2  # At least one constraint and one index
    ...
    async def test_create_cryptocurrency_node(self, mock_neo4j_driver):
        ...
        # Call the method
        await client.create_cryptocurrency_node("BTC", "Bitcoin", market_cap=1000000)
    ...
    async def test_create_correlation(self, mock_neo4j_driver):
        ...
        mock_session.run.assert_awaited_once()
        query = mock_session.run.await_args[0][0]
        # The query uses a single MATCH with a comma to match both nodes
        assert "MATCH (c1:Cryptocurrency {id: $symbol1}), (c2:Cryptocurrency {id: $symbol2})" in query
