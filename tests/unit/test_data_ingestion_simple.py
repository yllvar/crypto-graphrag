...
class TestDataIngestorSimple:
    ...
    @pytest.mark.asyncio
    async def test_initialize_success(self, mock_exchange_class, mock_exchange_instance, mock_engine, mock_db_connection):
        ...
        with (
            patch('ccxt.binance', mock_exchange_class),  # Mock the exchange class
            patch('src.data.ingestion.engine', mock_engine),  # Mock the SQLAlchemy engine
            patch('src.data.ingestion.redis_client.connect', AsyncMock(return_value=True)),  # Mock Redis connection
            ...
            with patch('src.data.ingestion.logger') as mock_logger:
                ...
                mock_logger.info.assert_called_once_with("Initialized binance exchange")
    ...
    @pytest.mark.asyncio
    async def test_initialize_with_exception(self, mock_exchange_class, mock_exchange_instance, mock_engine):
        """Test that exceptions during initialization are properly handled."""
        # Configure the mock exchange to raise an exception
        error_message = "Connection error"
        mock_exchange_instance.load_markets = AsyncMock(side_effect=Exception(error_message))
        mock_exchange_class.return_value = mock_exchange_instance
        ...
        with (
            ...
            patch('src.data.ingestion.redis_client.connect', AsyncMock(return_value=True)),
